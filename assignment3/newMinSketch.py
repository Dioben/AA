...
class CountMinSketch(object):
    ...
    def update(self, x):
        ...
        now only increments buckets matching smallest value
        '''
        self.n += 1
        indexes = [x for x in zip( range(self.d) , self._hash(x))]
        minval = min([ self.table[index] for index in indexes])
        indexes = [index for index in indexes if self.table[index]==minval]
